...
const source = await readFile(resolve(process.cwd(), "contracts/BasePalettePress.sol"), "utf8");
const input = {
  language: "Solidity",
  sources: { "BasePalettePress.sol": { content: source } },
  settings: { outputSelection: { "*": { "*": ["abi", "evm.bytecode.object"] } } },
};
const output = JSON.parse(solc.compile(JSON.stringify(input)));
const contract = output.contracts?.["BasePalettePress.sol"]?.BasePalettePress;
...
console.log(`Deploying BasePalettePress from ${account.address} on Base...`);
...
console.log("Set NEXT_PUBLIC_PALETTE_PRESS_CONTRACT_ADDRESS to this contract address in Vercel.");
